...
{
  string info_filename = "../00-data/02-merged/merged_structured_papers_info_mag.txt";
  ...
  string ref_filename = "../00-data/02-merged/merged_structured_papers_refs.txt";
  ...
  ifstream awp_rowptr("../00-data/03-crs/author_writes_paper_rowptr.txt");
  ifstream awp_colind("../00-data/03-crs/author_writes_paper_colind.txt");
  ...
  while(getline(awp_rowptr, row_line))
  {
    ...
    for (long long int i = rowptr_start; i < rowptr_end; i++)
    {
      ...
      authorpapers.insert(paper);
    }

    for(set<int>::iterator it = authorpapers.begin(); it != authorpapers.end(); ++it)
    {
      int paper = *it;

      for (int j = 0; j < paper_refs[paper].size(); j++)
      {
        if (authorpapers.find(paper_refs[paper][j]) != authorpapers.end())
          continue;
